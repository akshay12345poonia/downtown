  ...
  />
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          >
            ...
            <div
              className={`absolute left-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl border py-3 transition-all duration-300 ${
                companyOpen
          ...
          {!isLoggedIn ? (
            ...
            <div className="relative">
              <button
                ...
                className="w-10 h-10 rounded-full bg-brand p-0.5 shadow-lg hover:scale-105 transition"
              >
              ...
              <div
                className={`absolute right-0 mt-4 w-64 bg-white rounded-2xl shadow-2xl border py-4 transition-all duration-300 ${
                  profileOpen
                    ? 'opacity-100 visible translate-y-0'
                    : 'opacity-0 invisible -translate-y-4'
                }`}
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden fixed inset-0 z-[60] bg-black transition-transform duration-500 ${
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-8">
            {!isLoggedIn ? (
              <>
                <Link to="/signin" onClick={() => setMobileMenuOpen(false)} className="btn btn-secondary w-full mb-4">
                  Sign In
                </Link>
                <Link to="/signup" onClick={() => setMobileMenuOpen(false)} className="btn btn-primary w-full">
                  Get Started
                </Link>
              </>
            ) : (
              <button onClick={handleLogout} className="btn bg-red-500 text-white w-full">
                Sign Out
              </button>
            )}
          </div>
